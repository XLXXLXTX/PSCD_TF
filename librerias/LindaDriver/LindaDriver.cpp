...
LindaDriver::LindaDriver(string SERVER_ADDRESS, string SERVER_PORT){
    ...
    if (read_bytes == -1) {     //caso error del Recv
        cerr << "Error PN read_bytes: " + strerror(errno) + "\n";
    ...
    if(send_bytes == -1){   //caso error del Send
        cerr << "Error cerrar socket_despliegue send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        chan->Close(socket_despliegue);
        exit(1);
    }
...
void LindaDriver::PN(Tupla t){
    ...
    if (send_bytes == -1) {  // Caso error del Send
        cerr << "Error PN send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        cierreSockets();
        exit(3);
    }
...
void LindaDriver::RN(Tupla p, Tupla& t){
    ...
    int send_bytes = LS->Send(socket_fd, mensaje);  // Envíamos el string
    if (send_bytes == -1) { //caso error del Send
        cerr << "Error RN send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        cierreSockets();
        exit(3);
    }
    ...
    int read_bytes = LS->Recv(socket_fd, respuesta, MESSAGE_SIZE);
    if (read_bytes == -1) {  //caso error del Recv
        cerr << "Error RN read_bytes: " + strerror(errno) + "\n";
        // Cerramos los sockets
        cierreSockets();
        exit(4);
    }
...
void LindaDriver::RN_2(Tupla p1, Tupla p2, Tupla& t1, Tupla& t2){
    ...
    int send_bytes = LS->Send(socket_fd, mensaje);	// Envíamos el string
    if (send_bytes == -1) {  //caso error del Send
        cerr << "Error RN_2 send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        cierreSockets();
        exit(3);
    }
    ...
    int read_bytes = LS->Recv(socket_fd, respuesta, MESSAGE_SIZE);
    if (read_bytes == -1) {   //caso error del Recv
        cerr << "Error RN_2 read_bytes: " + strerror(errno) + "\n";
        // Cerramos los sockets
        cierreSockets();
        exit(4);
    }
...
void LindaDriver::RDN(Tupla p, Tupla& t){
    ...
    int send_bytes = LS->Send(socket_fd, mensaje);	// Envíamos el string
    if (send_bytes == -1) {   //caso error del Send
        cerr << "Error RDN send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        cierreSockets();
        exit(3);
    }
    ...
    int read_bytes = LS->Recv(socket_fd, respuesta, MESSAGE_SIZE);
    if (read_bytes == -1) { //caso error del Recv
        cerr << "Error RDN read_bytes: " + strerror(errno) + "\n";
        // Cerramos los sockets
        cierreSockets();
        exit(4);
    }
...
void LindaDriver::RDN_2(Tupla p1, Tupla p2, Tupla& t1, Tupla& t2) {
    ...
    int send_bytes = LS->Send(socket_fd, mensaje);  // Envíamos el string
    if (send_bytes == -1) {  // Caso error del Send
        cerr << "Error RDN_2 send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        cierreSockets();
        exit(3);
    }
    ...
    int read_bytes = LS->Recv(socket_fd, respuesta, MESSAGE_SIZE);
    if(read_bytes == -1) {   // Caso error del Recv
        cerr << "Error RDN_2 read_bytes: " + strerror(errno) + "\n";
        // Cerramos los sockets
        cierreSockets();
        exit(4);
    }
...
void LindaDriver::cierreSockets() {
    ...
    if (send_bytes == -1) {   // Caso error del Send
        cerr << "Error cerrar socket_LS1 send_bytes: " + strerror(errno) + "\n";
        // Cerramos los sockets
    ...
    if (send_bytes == -1) { // Caso error del Send
        cerr << "Error cerrar socket_LS2 send_bytes: " + strerror(errno) + "\n";
        // Cerramos los sockets
        LS2->Close(socket_LS2);
        LS3->Close(socket_LS3);
        exit(1);
    ...
    if (send_bytes == -1) {   // caso error del Send
        cerr << "Error cerrar socket_LS3 send_bytes: " + strerror(errno) + "\n";
        // Cerramos el socket
        LS3->Close(socket_LS3);
        exit(1);
    }
